...
fn bench_matrix_exponential(c: &mut Criterion) {
    ...
    for dim in dimensions {
        group.bench_with_input(BenchmarkId::from_parameter(format!("{}x{}", dim, dim)), &dim, |b, &dim| {
            ...
            b.iter(|| {
                utils::matrix_exponential(black_box(&m))
            });
        });
    }
...
fn bench_acyclicity_constraint(c: &mut Criterion) {
    ...
    for dim in dimensions {
        group.bench_with_input(BenchmarkId::from_parameter(format!("{}nodes", dim)), &dim, |b, &dim| {
...
fn bench_l1_penalty(c: &mut Criterion) {
    ...
    for dim in dimensions {
        group.bench_with_input(BenchmarkId::from_parameter(format!("{}x{}", dim, dim)), &dim, |b, &dim| {
            ...
            b.iter(|| {
                scoring::l1_penalty(black_box(&w))
            });
        });
    }
