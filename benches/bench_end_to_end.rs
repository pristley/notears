    ...
    for d in dimensions {
        group.bench_with_input(
            ...
            |b, &d| {
                ...
                let (_w_true, data) =
                    profiling_utils::generate_erdos_renyi_dag(d, n_edges, n_samples);
                let standardized = utils::standardize_data(&data).unwrap();
...
fn profile_optimization_verbose(d: usize, n: usize, lambda: f64) {
    ...
    let (_w_true, data) =
        profiling_utils::generate_erdos_renyi_dag(d, (d as f64 * 1.5) as usize, n);
    ...
    println!(
        "Time per iteration: {:.3}s",
        solve_time / result.iterations as f64
    );
}
